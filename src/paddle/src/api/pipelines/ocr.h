...
class My_PaddleOCR {
public:
  My_PaddleOCR(const PaddleOCRParams &params = PaddleOCRParams());

  std::vector<std::unique_ptr<BaseCVResult>> Predict(const std::string &input){
    std::vector<std::string> inputs = {input};
    return std::vector<std::unique_ptr<BaseCVResult>>();
  };
  ...
  std::vector<std::unique_ptr<BaseCVResult>> Predict(const std::vector<cv::Mat> &inputs) {
    return pipeline_infer_->Predict(inputs);
  }
  ...
  void CreatePipeline() {
    pipeline_infer_ = std::unique_ptr<BasePipeline>(
        new My_OCRPipeline(ToOCRPipelineParams(params_)));
  }
